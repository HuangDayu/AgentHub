...
import io.agentscope.core.tracing.telemetry.model.*;
...
import static io.agentscope.core.tracing.telemetry.AgentScopeIncubatingAttributes.*;
...
import static io.agentscope.core.tracing.telemetry.GenAiIncubatingAttributes.GenAiOperationNameIncubatingValues.*;
import static io.agentscope.core.tracing.telemetry.GenAiIncubatingAttributes.GenAiProviderNameIncubatingValues.*;
...
final class AttributesExtractors {
    ...
    static Attributes getCommonAttributes() {
        ...
        Optional<TenantThreadContext> current = TenantContextHolder.current();
        current.ifPresent(tenantThreadContext -> {
            internalSet(builder, AGENTHUB_TENANT_ID, tenantThreadContext.getTenantId());
            internalSet(builder, AGENTHUB_WORKSPACE_ID, tenantThreadContext.getWorkspaceId());
            internalSet(builder, AGENTHUB_AGENT_ID, tenantThreadContext.getAgentId());
            internalSet(builder, AGENTHUB_SESSION_ID, tenantThreadContext.getSessionId());
        });
        return builder.build();
    }
