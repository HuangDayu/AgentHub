...
import com.agenthub.infrastructure.tools.system_tools.core_tools.dto.FileContentResult;
...
/**
 * 文件操作工具，提供文件的读取、写入、编辑、补丁应用和目录操作功能。
 */
@RequiredArgsConstructor
@AgentTools(name = "FileTools", description = "文件操作工具，提供文件的读取、写入、编辑、补丁应用和目录操作功能")
public class FileTools {
    ...
    public FileContentResult read(
        ...
        if (!Files.exists(path)) {
            return new FileContentResult(false, filePath, "", "文件不存在", 0);
        }
    ...
    public FileContentResult edit(
        ...
        if (!content.contains(oldContent)) {
            return new FileContentResult(false, filePath, content, "未找到要替换的内容", 0);
        }
    ...
    public FileContentResult append(
        ...
        return new FileContentResult(true, filePath, fullContent, "内容追加成功", fullContent.length());
    }
    ...
    public List<String> listDirectory(
        ...
        return Files.list(Paths.get(dirPath))
                ...
                .map(p -> {
                    ...
                    try {
                        return name + " (" + Files.size(p) + " bytes)";
                    } catch (IOException e) {
                        return name;
                    }
                }).toList();
