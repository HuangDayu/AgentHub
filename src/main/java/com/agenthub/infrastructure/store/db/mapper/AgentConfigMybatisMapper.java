...
public interface AgentConfigMybatisMapper extends BaseMapper<AgentConfigEntity> {
    @Select("SELECT * FROM agent_config WHERE agent_id = #{agentId} ORDER BY priority ASC, created_at ASC")
    List<AgentConfigEntity> selectByAgentId(@Param("agentId") String agentId);

    @Select("SELECT * FROM agent_config WHERE agent_id = #{agentId} AND category = #{category} ORDER BY priority ASC")
    List<AgentConfigEntity> selectByAgentIdAndCategory(@Param("agentId") String agentId, @Param("category") String category);

    @Select("SELECT * FROM agent_config WHERE agent_id = #{agentId} AND type = #{type}")
