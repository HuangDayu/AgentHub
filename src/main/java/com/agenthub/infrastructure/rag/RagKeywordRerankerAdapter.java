 ...
 */
@Component
@ConditionalOnProperty(name = "agenthub.rag.reranker.type", havingValue = "keyword", matchIfMissing = true)
public class RagKeywordRerankerAdapter implements RagRerankerPort {

    private static final Logger log = LoggerFactory.getLogger(RagKeywordRerankerAdapter.class);
    ...
    public List<RetrievalResult> rerank(String queryText, List<RetrievalResult> results) {
        ...
        log.debug("Reranking {} results with {} prompt keywords: {}", results.size(), queryKeywords.size(), queryKeywords);
