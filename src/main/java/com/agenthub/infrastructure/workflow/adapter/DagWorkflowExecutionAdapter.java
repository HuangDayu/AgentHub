...
import com.agenthub.infrastructure.workflow.engine.DagWorkflowEngine;
...
public class DagWorkflowExecutionAdapter implements DagWorkflowExecutionPort {

    private final DagWorkflowEngine workflowEngine;
    private final DagWorkflowStateManager stateManager;
    private final DagWorkflowRepository workflowRepository;
     ...
     */
    @Override
    public Mono<DagWorkflowContext> initializeContext(ExecutionCommand command) {
     ...
     * @return 执行上下文
     */
    private DagWorkflowContext createContext(ExecutionCommand command) {
        DagWorkflowContext context = DagWorkflowContext.create(command.getWorkflowId());
     ...
     * 从数据库加载工作流图。
     */
    private DagWorkflowGraph loadGraph(String workflowId) {
        return workflowRepository.findById(workflowId)
            .map(DagWorkflow::getGraphDefinition)
     ...
     */
    @Override
    public Flux<NodeResult> executeNode(DagWorkflowContext context, String nodeId) {
     ...
     */
    @Override
    public Flux<NodeResult> executeWorkflow(DagWorkflowContext context) {
     ...
     * @return 节点执行结果流
     */
    private Flux<NodeResult> executeWithLifecycle(DagWorkflowContext context) {
     ...
     * @return 执行上下文
     */
    private Mono<DagWorkflowContext> startExecution(DagWorkflowContext context) {
        return stateManager.updateStatus(context.getExecutionId(), DagWorkflowStatus.EXECUTING)
            .thenReturn(context);
    }
     ...
     * @return 节点执行结果流
     */
    private Flux<NodeResult> runWorkflow(DagWorkflowContext context) {
        return workflowEngine.execute(context)
     ...
     * @param result 节点结果
     */
    private void saveNodeResult(DagWorkflowContext context, NodeResult result) {
     ...
     * @param context 执行上下文
     */
    private void endExecution(DagWorkflowContext context) {
        DagWorkflowStatus finalStatus = determineFinalStatus(context);
     ...
     * @return 最终状态
     */
    private DagWorkflowStatus determineFinalStatus(DagWorkflowContext context) {
        boolean hasFailure = context.getNodeResults().values().stream()
            .anyMatch(result -> !result.isSuccess());
        return hasFailure ? DagWorkflowStatus.FAILED : DagWorkflowStatus.SUCCESS;
    }
