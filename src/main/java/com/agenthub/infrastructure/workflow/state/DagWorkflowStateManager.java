...
public class DagWorkflowStateManager {
     ...
     * @return 保存结果的Mono
     */
    public Mono<Void> saveContext(DagWorkflowContext context) {
     ...
     * 更新context的状态。
     */
    private DagWorkflowContext updateContextStatus(DagWorkflowContext context, String statusStr) {
        if (statusStr != null && !statusStr.isEmpty()) {
            try {
                context.setStatus(DagWorkflowStatus.valueOf(statusStr));
            } catch (Exception e) {
     ...
     * 处理状态加载错误。
     */
    private Mono<DagWorkflowContext> handleStatusLoadError(DagWorkflowContext context, String executionId, Throwable e) {
     ...
     * @return JSON字符串的Mono
     */
    private Mono<String> serializeContext(DagWorkflowContext context) {
     ...
     * @return 执行上下文的Mono
     */
    private Mono<DagWorkflowContext> deserializeContext(String json) {
        return Mono.fromCallable(() -> objectMapper.readValue(json, DagWorkflowContext.class));
    }
