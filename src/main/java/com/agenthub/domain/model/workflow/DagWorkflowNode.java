...
public class DagWorkflowNode {
    ...
    @JsonCreator
    public DagWorkflowNode(@JsonProperty("id") String id,
                       @JsonProperty("type") DagNodeType type) {
        this.id = id;
        this.type = type;
    }
     ...
     * @return 工作流节点实例
     */
    public static DagWorkflowNode create(DagNodeType type, String name) {
        DagWorkflowNode node = new DagWorkflowNode(randomId(), type);
        node.name = name;
     ...
     * @param newStatus 新状态
     */
    public void updateStatus(DagNodeStatus newStatus) {
