...
public class WorkflowGraph {
    ...
    public WorkflowGraph addNode(WorkflowNode node) {
        List<WorkflowNode> newNodes = new ArrayList<>(nodes);
        newNodes.add(node);
        return new WorkflowGraph(List.copyOf(newNodes), edges);
    }
     ...
     * @return 新的工作流图
     */
    public WorkflowGraph addEdge(WorkflowEdge edge) {
        List<WorkflowEdge> newEdges = new ArrayList<>(edges);
        newEdges.add(edge);
        return new WorkflowGraph(nodes, List.copyOf(newEdges));
    }
}
